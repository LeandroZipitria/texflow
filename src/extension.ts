...
function isSafeParagraph(raw: string): boolean {
  ...
  if (/\\(begin|end|input|include|hypertarget|label|only|visible|uncover|pause|vspace|centering|includegraphics|tikz|footnote|href|url|index|nomenclature)/.test(t)) return false;
  return true;
}
...
function serializeBlock(block: ParsedBlock, payload: any): string {
  if (block.kind === 'paragraph') { const text=normalizeEditableText(payload.text); const requested=String(payload.align||''); if(requested==='justify') return text+'\n'; const env=requested==='left'?'flushleft':requested==='right'?'flushright':requested==='center'?'center':(['flushleft','center','flushright'].includes(String(block.env||''))?String(block.env):''); return env?`\\begin{${env}}\n${text}\n\\end{${env}}`:text+'\n'; }
...
function latexToHtml(text){
 ...
 const inlineMath=[],citations=[],references=[],hspaces=[],footnotes=[],links=[],indexes=[],fields=[],nomenclatures=[];
...
function renderDocumentOutline(nav){
 ...
 flow.forEach(x=>{
  ...
  if(x.kind==='bibliography'){found=true;const row=document.createElement('div');row.className='doc-outline doc-outline-bibliography';row.textContent='Bibliography';row.onclick=()=>{const el=document.getElementById(x.id);if(el)el.scrollIntoView({behavior:'smooth',block:'start'});};nav.appendChild(row);return;}
...
function normalizeAtomicInsertionRange(editable,range){
 ...
 const atomic=startEl&&startEl.closest?startEl.closest('.bib-citation,.tex-reference,.inline-math,.tex-footnote,.tex-link,.tex-index,.tex-field,.tex-nomenclature,.math-caret-anchor'):null;
 if(atomic&&editable.contains(atomic)){
  let boundary=atomic;
  if((atomic.classList.contains('bib-citation')||atomic.classList.contains('tex-reference')||atomic.classList.contains('inline-math'))&&atomic.nextSibling&&atomic.nextSibling.nodeType===Node.ELEMENT_NODE&&atomic.nextSibling.classList.contains('math-caret-anchor'))boundary=atomic.nextSibling;
