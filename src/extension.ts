...
function isAtomicDocumentBlockNode(x){if(!(x&&x.kind==='block'&&x.block))return false;if((x.block.kind==='comment'||x.block.kind==='commentblock')&&!reviewBlockVisible(x.block))return false;return ['equation','figure','table','vspace','columns','quote','container','break','theorem','customenv','comment','commentblock'].includes(x.block.kind);}
