...
function commentMarkerHtml(key,nodeId,kind,title){
 ...
 return '<div class="comment-anchor" data-review-key="'+esc(key)+'" data-node-id="'+esc(nodeId||'')+'"><button type="button" class="comment-marker '+cls+'" data-review-key="'+esc(key)+'" data-node-id="'+esc(nodeId||'')+'" title="'+esc(title||'Comment')+'" aria-label="'+esc(title||'Comment')+'">'+glyph+'</button></div>';
}
...
function parseDocumentFlow(){
 ...
 documentRefs={};documentFlowById={};documentLabels=[];applyDocumentHeadingNumbers(out);let equationNumber=0;
...
function documentBlockHtml(node){
 ...
 if(b.kind==='comment'){const reviewKey=reviewDocumentKey(node);if(!reviewBlockVisible(b)||reviewItemHidden(reviewKey))return '';const kind=b.commentTag==='TODO'?'todo':b.commentTag==='FIXME'?'fixme':(b.commentNote?'authornote':'comment'),title=b.commentTag==='TODO'?'TODO':b.commentTag==='FIXME'?'FIXME':(b.commentNote?'Author note':'Source comment');return commentMarkerHtml(reviewKey,node.id,kind,title);}
...
function isAtomicDocumentBlockNode(x){if(!(x&&x.kind==='block'&&x.block))return false;if((x.block.kind==='comment'||x.block.kind==='commentblock')&&!reviewBlockVisible(x.block))return false;return ['equation','figure','table','tikz','vspace','columns','quote','container','break','theorem','customenv','comment','commentblock'].includes(x.block.kind);}
