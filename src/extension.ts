...
.multi-figure-grid{width:100%;display:flex;gap:10px;align-items:flex-start;justify-content:center;flex-wrap:nowrap}.multi-figure-grid.layout-stacked{flex-direction:column;align-items:center}.multi-figure-item{flex:0 1 var(--item-container,48%);width:var(--item-container,48%);min-width:0;border:1px solid var(--line);border-radius:7px;padding:7px;background:color-mix(in srgb,var(--paper) 97%,var(--fg) 3%)}.multi-figure-grid.layout-stacked .multi-figure-item{width:min(94%,680px);flex-basis:auto}.multi-figure-preview{display:flex;justify-content:center;align-items:center;min-height:90px}.multi-figure-media{position:relative;display:inline-block;min-width:28px;max-width:100%}.multi-figure-media img,.multi-figure-media object{display:block;width:100%;height:auto;max-width:100%;max-height:260px;object-fit:contain;border:0;background:transparent;pointer-events:none}.multi-figure-tools{display:grid;grid-template-columns:minmax(0,1fr) 86px auto;gap:6px;align-items:center;margin-top:6px;font-size:10px;color:var(--muted)}.multi-figure-path{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:var(--vscode-editor-font-family,monospace);font-size:.9em;opacity:.86}.multi-figure-tools label{display:flex;align-items:center;gap:3px;white-space:nowrap;min-width:0}.multi-figure-tools input{padding:2px 4px;background:var(--input);color:var(--fg);border:1px solid var(--line-strong);border-radius:4px;font:inherit}.multi-figure-tools input.multi-figure-width{width:60px;min-width:60px}.multi-figure-tools button{padding:3px 6px;border:1px solid var(--line-strong);border-radius:5px;background:var(--input);color:var(--fg);font:inherit;cursor:pointer}.multi-figure-tools button:hover{background:var(--hover)}.multi-figure-subcaption{display:flex;align-items:center;gap:5px;margin-top:5px;font-size:10px;color:var(--muted)}.multi-figure-subcaption span{flex:0 0 auto}.multi-figure-subcaption input{min-width:0;flex:1;padding:3px 5px;background:var(--input);color:var(--fg);border:1px solid var(--line-strong);border-radius:4px;font:inherit}
...
function documentFigureHtml(node,b){
 ...
 const imageSizing=isTikz?'':'<label>W <input class="figure-width-input" type="number" min="5" max="100" step="1" value="'+Math.round(width)+'">%</label><label>Rotate <input class="figure-angle-input" type="number" min="-360" max="360" step="1" value="'+angle+'">°</label>';
...
function serializeDocumentTable(node,payload){
 ...
 const placement=String(payload.placement??b.tablePlacement??'').trim(),tableStyle=String(payload.tableStyle??b.tableStyle??'plain')==='booktabs'?'booktabs':'plain',tableSize=['normalsize','small','footnotesize','scriptsize','tiny'].includes(String(payload.tableSize??b.tableSize??''))?String(payload.tableSize??b.tableSize??''):'',caption=escapeLatexPlainText(String(payload.caption??b.tableCaption??'').trim().replace(/[{}]/g,'')),label=String(payload.label??b.tableLabel??'').trim().replace(/[{}\\\s]/g,''),captionPosition=String(payload.captionPosition??b.tableCaptionPosition??'below')==='above'?'above':'below';
 const lines=['\\begin{table}'+(placement?'['+placement+']':''),'\\centering'];if(tableSize)lines.push('\\'+tableSize);if(caption&&captionPosition==='above')lines.push('\\caption{'+caption+'}');if(caption&&captionPosition==='above'&&label)lines.push('\\label{'+label+'}');lines.push('\\begin{tabular}{'+columns.join('')+'}');if(tableStyle==='booktabs')lines.push('\\toprule');
...
function renderTable(block,frameIndex,wrap){
 const tableSize=String(block.tableSize||'');wrap.className+=' table-card'+(tableSize?' table-size-'+tableSize:'');const cols=block.tableColumns||[],rows=block.tableRows||[],caption=block.tableCaption||'',label=block.tableLabel||'',captionPosition=block.tableCaptionPosition||'below';
 const colTools=cols.map((c,i)=>'<label>C'+(i+1)+' <select class="table-col-align" data-col="'+i+'"><option value="l"'+(c==='l'?' selected':'')+'>L</option><option value="c"'+(c==='c'?' selected':'')+'>C</option><option value="r"'+(c==='r'?' selected':'')+'>R</option></select></label>').join('');
...
function renderFigure(block,frameIndex,wrap){
 ...
 const imageTools=isTikz||multi?'':'<label><input class="figure-lock" type="checkbox" checked> lock</label><label>W <input class="figure-width-input" type="number" min="5" max="100" step="1" value="'+Math.round(width)+'">%</label><label class="height-control" style="display:none">H <input class="figure-height-input" type="number" min="5" max="100" step="1" value="'+Math.round(height||40)+'">%</label>';
